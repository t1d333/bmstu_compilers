...
class ParsingTable:
    ...
    def __setup_from_grammar(self, gr):
        ...
        self.goto = tuple(
            {x: None for x in self.nonterms} for i in range(self.n_states)
        )
        self.action = tuple(
            {x: set() for x in self.terminals} for i in range(self.n_states)
        )
        ...
        for state_id in range(self.n_states):
            for item, next_symbol in self.__ccol[state_id]:
                ...
                if dot < len(pbody):
                    terminal = pbody[dot]
                    if (
                        not isinstance(terminal, BaseTerminal)
                        or terminal not in goto_precalc[state_id]
                    ):
                        continue
    ...
    def __stringify_lr_zero_item(self, item):
        ...
        return RULE_INDEXING_PATTERN % (
            prod_index,
            pname.name + ": " + dotted_pbody_str,
        )
    ...
    def stringify_state(self, state_id):
        ...
        gotos = [
            (nt, sid) for nt, sid in self.goto[state_id].items() if sid is not None
        ]
...
def get_canonical_collection(gr):
    ...
    table = [
        {item: LrZeroItemTableEntry() for item in kstates[i]} for i in range(n_states)
    ]
...
class ParseError(Error):
    ...
    def message(self):
        ...
        return f"Неожиданный символ {self.unexpected}, " + f"ожидалось {expected}"
...
class Parser(object):
    ...
    def stringify(self, indexes=True):
        ...
        if indexes:
            lines = "\n".join(
                RULE_INDEXING_PATTERN % (x, y) for x, y in enumerate(lines.split("\n"))
            )
        return lines
        ...
        while True:
            ...
            match action:
                ...
                case Reduce(rule):
                    ...
                    attrs = [
                        attr
                        for state, coord, attr in stack[len(stack) - n :]
                        if attr != None
                    ]
                ...
                case None:
                    expected = [
                        symbol
                        for symbol, actions in self.table.action[cur_state].items()
                        if len(actions) > 0
                    ]
                    raise ParseError(
                        pos=cur.pos.start, unexpected=cur, expected=expected
                    )
...
def kernels(item_set):
    return frozenset(
        (item, nextsym) for item, nextsym in item_set if item[1] > 0 or item[0] == 0
    )
...
class Lexer:
    ...
    def __init__(self, domains, text, skip):
        ...
        self.domains += [
            Terminal("-skip-", regex, lambda _: self.skip_token) for regex in skip
        ]
    ...
    def next_token(self):
        while self.pos.offset < len(self.text):
            ...
            matches = [
                (d, d.priority, *d.match(self.text, offset)) for d in self.domains
            ]
